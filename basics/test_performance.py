...
tf_levels = z3.Strings('tf_lv_0 tf_lv_1 tf_lv_2 '
                       'tf_lv_3 tf_lv_4 tf_lv_5 '
                       'tf_lv_6 tf_lv_7')

qmarks = z3.Strings('qmark_0 qmark_1 qmark_2')
stars = z3.Strings('star_0 star_1')

def print_model(model):
  for k,v in sorted([(k, model[k]) for k in model], key = lambda x: str(x[0])):
    print(f'\t{k}\t->\t{v}')

def get_concat_for_topic(level):
  if level == 0:
...
def get_base_solver(level):
  solver = z3.Solver()
  # Each '?' should be a single char
  for qmark in qmarks:
    solver.add(z3.Length(qmark) == 1)
    
  # Id 1
  solver.add(z3.And(id1 == z3.Concat('badgeReader', qmarks[0], qmarks[1]),
                    z3.Not(False)))
  # Id 2
  # solver.add(z3.And(id2 == stars[0],
  #                   z3.Not(False)))
  # Pub
  solver.add(z3.And(topic == z3.Concat('physicalAC/floor1/', id1, '/check'),
                    z3.Not(False)))
  # Rec
  # solver.add(z3.And(topic == stars[1],
  #                   z3.Not(False)))
  # Sub
  # solver.add(z3.And(topic_filter == z3.Concat('physicalAC/floor', qmarks[2], '/detectedMovement/', id2),
  #                   z3.Not(False)))

  #TODO: this is still very slow
  # Topic (and topic filter) levels cannot have '/'
  for i in range(level+1):
    # solver.add(z3.InRe(topic_levels[i], RE_QMARK_CHARS_ONLY))
    solver.add(z3.Not(z3.Contains(topic_levels[i], '/')))
    # solver.add(z3.Not(z3.Contains(tf_levels[i], '/')))
...
for level in range(8):
  s_topic_lvls = get_base_solver(level)
  s_topic_lvls.add(topic == get_concat_for_topic(level))
  # s_topic_lvls.add(z3.Not(z3.Or(z3.Contains(topic_levels[0], '/'),
  #                               z3.Contains(topic_levels[1], '/'),
  #                               z3.Contains(topic_levels[2], '/'))))
                                # z3.Contains(topic_levels[3], '/'),
                                # z3.Contains(topic_levels[4], '/'),
                                # z3.Contains(topic_levels[5], '/'),
                                # z3.Contains(topic_levels[6], '/'),
                                # z3.Contains(topic_levels[7], '/'))))

  start_time = time.time()
  if s_topic_lvls.check() == z3.unsat:
    print(f'T:{time.time() - start_time} -- #{level+1} topic levels is {z3.unsat}')
    continue
  elif s_topic_lvls.check() == z3.unknown:
    print(f'T:{time.time() - start_time} -- #{level+1} topic levels is {z3.unknown}')
    continue
  print(f'T:{time.time() - start_time} -- #{level+1} topic levels is {z3.sat}')
  print_model(s_topic_lvls.model())
