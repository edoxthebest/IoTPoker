...
def get_topic_for_level(level):
  ...
  topic = [topic_levels[0]]
  for topic_level in range(level):
    topic.append('/')
    topic.append(topic_levels[topic_level + 1])
  return z3.Concat(topic)
...
def get_constraints_for_topic_lvls(topic_lvls):
  topic = get_topic_for_level(topic_lvls)
  cons_for_level = []
  
  # solver.add(z3.Length(topic) < 30)

  for i in range(topic_lvls+1):
    cons_for_level.append(z3.InRe(topic_levels[i], RE_STAR_CHARS_ONLY))
  
  # ID1
  cons_for_level.append(z3.And(z3.InRe(id1, z3.Concat(z3.Re("badgeReader"),
                                      RE_QMARK, RE_QMARK)),
              z3.Not(z3.InRe(id1, z3.Empty(z3.ReSort(z3.StringSort()))))))
  # ID2
  cons_for_level.append(z3.And(z3.InRe(id2, RE_STAR), #z3.Re('aaa')),
              z3.Not(z3.InRe(id2, z3.Empty(z3.ReSort(z3.StringSort()))))))
  # PUB
  cons_for_level.append(z3.And(z3.InRe(topic,z3.Concat(z3.Re("physicalAC/floor1/"), z3.Re(id1), z3.Re("/check"))),
              z3.Not(z3.InRe(topic, z3.Empty(z3.ReSort(z3.StringSort()))))))
  
  # REC
  cons_for_level.append(z3.And(z3.InRe(topic, RE_STAR),
              z3.Not(z3.InRe(topic, z3.Empty(z3.ReSort(z3.StringSort()))))))
  # SUB
  cons_for_level.append(z3.And(z3.InRe(topic_filter, z3.Concat(z3.Re('physicalAC/floor'), RE_QMARK, z3.Re('/detectedMovement/'), z3.Re(id2))),
              z3.Not(z3.InRe(topic_filter, z3.Empty(z3.ReSort(z3.StringSort()))))))
  
  
  return cons_for_level
...
t = time.time()
for tf_level in range(8):
  s = z3.Solver()
  
  cons_for_level = []
  for topic_level in range(tf_level, 8):
    cons = get_constraints_for_topic_lvls(topic_level)
    
    base = []
    for i in range(tf_level):
      base.append(z3.Union(z3.Re(topic_levels[i]), 
                           z3.Re('+')))
      base.append(z3.Re('/'))
      
    if topic_level == tf_level:
      base.append(z3.Union(z3.Re(topic_levels[topic_level]),
                           z3.Re('+'),
                           z3.Re('#')))
    else:
      base.append(z3.Re('#'))
    
    cons.append(z3.InRe(topic_filter, z3.Concat(base) if tf_level != 0 else base[0]))
    cons_for_level.append(z3.And(cons))
    
  s.add(z3.Or(cons_for_level))
  ...
  # CASE1: same length
  start_time = time.time()
  s_case_1 = get_constraints_for_topic_lvls()
  s_case_1.add(topic == get_topic_for_level(topic_level))
